...
| Cross-domain startup bundle integrity. | `f5-domain-preflight.py` requires three clean fixture result/state triplets and three clean worker results, matches state hashes and the exact 1,320/33-worker candidate, validates 33 unique accepted worker incarnations and six distinct bounded Linux Docker Engine identities, rejects reused advertised hosts/endpoints/engines/incarnations or image drift, and emits a hashed-input report that explicitly disclaims physical-host and capacity certification. Real six-engine independent-host inputs remain pending. |
