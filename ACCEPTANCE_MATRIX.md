...
| Boot and validate the stack. | The environment-gated startup workflow boots app/worker services, drives create/start/validate/stop through HTTP, proves normal-feed video/audio/captions, and captures Docker logs. Fixture-domain startup checks exact inventory, API, sampled media, resources, and logs; its marked fault workflow also proves sampled media failure and recovery across a real container stop/restart. Worker-domain startup validates the candidate zone, 11 certificate identities, immutable image resolution, host/worker health APIs, stable zero-restart processes, and logs. The runbook retains the cross-domain Chrome check. |
