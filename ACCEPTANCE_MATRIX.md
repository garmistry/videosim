...
| Boot and validate the stack. | The environment-gated startup workflow boots app/worker services, drives create/start/validate/stop through HTTP, proves normal-feed video/audio/captions, and captures Docker logs. Fixture-domain startup checks exact inventory, API, sampled media, resources, and logs; its marked fault workflow also proves sampled media failure and recovery across a real container stop/restart. The durable fixture gate additionally boots fresh PostgreSQL, imports a distinct-path catalog, starts real workers, and fails closed on lease/media/alarm/API/container/log mismatches. Worker-domain startup validates the candidate zone, 11 certificate identities, immutable image resolution, host/worker health APIs, stable zero-restart processes, and logs. The runbook retains the Chrome checks. |
...
| Capability | Acceptance evidence |
|---|---|
| Fixture catalog startup at the F5 candidate shape. | `import-fixture-scenario` validates the composed state, rejects shared endpoints or unrelated durable rows, atomically imports complete external-feed configurations, rereads exact parity, and writes a report that explicitly disclaims media/capacity certification. Marked PostgreSQL tests prove rollback, idempotence, and real scheduler lease output; a local booted API returned exact 1,320-row/660-SRT/660-DASH parity in seven pages. Independent-host media evidence remains pending. |
| One-domain durable media at the F5 shard shape. | A digest-pinned same-host run booted one 220-SRT/220-DASH fixture shard, fresh PostgreSQL, the API, and 11 workers; all 440 latest results matched declared behavior, leases balanced at 40 per worker, alarms/API parity matched exactly, byte-backed SRT captions passed, and all 15 containers remained stable with final resource/log capture. This is one-domain readiness, not F5 admission. |
| Cross-domain startup bundle integrity. | `f5-domain-preflight.py` requires three clean fixture result/state triplets, three clean worker results, and their three ordered raw worker Docker-stats files; it matches state hashes and the exact 1,320/33-worker candidate, validates 33 unique accepted worker incarnations, 11 valid resource rows and a matching SHA-256 per worker domain, and six distinct bounded Linux Docker Engine identities, rejects reused advertised hosts/endpoints/engines/incarnations, state/image/resource drift, or malformed/incomplete snapshots, and emits a hashed-input report that explicitly disclaims physical-host and capacity certification. Real six-engine independent-host inputs remain pending. |
| Worker-domain authority recovery at the F5 candidate shape. | `worker-domain-fault.py` waits for exact 33-worker/1,320-stream authority, hard-stops one 11-worker Compose domain, times all 440 replacement authorities, rejects healthy-owner churn during loss, holds recovery, restarts/rejoins the domain, and holds all 22 survivor incarnations stable for 30 seconds while checking target logs. A retained marked same-engine run passed at 36.233-second p95 and 37.315-second p99; the operator API returned 33 workers at 40 assignments each with clean spools, and peer-log window audit passed. Independent hosts, mTLS, real media/headroom, and 24-hour admission remain pending. |
...
| Durable control-plane load at the F5 candidate shape. | `control-plane-load` uses an empty disposable PostgreSQL database to sustain the exact workload's leases, heartbeats, profile-cadenced fenced reports, current state, and accepted-result/alarm outbox evidence. P0 coverage proves one 1,320-stream/33-worker tick, compressed real-expiry recovery from one 11-worker loss, and synthetic all-stream endpoint-fault raise/recovery with exact 1,320 raised, 1,320 cleared, and 2,640 alarm-outbox transitions. Retained evidence with the configured 30-second candidate default moved all 440 affected owners to 22 survivors at 29.888/29.889-second p95/p99, kept healthy ownership stable, and fenced the stale failed-owner report. Independent media, real endpoint faults, physical domains, and 24-hour admission remain pending. |
