...
| Cross-domain startup bundle integrity. | `f5-domain-preflight.py` requires three clean fixture result/state triplets and three clean worker results, matches state hashes and the exact 1,320/33-worker candidate, rejects reused advertised hosts/endpoints or image/identity drift, and emits a hashed-input report that explicitly disclaims physical-host and capacity certification. Real independent-host inputs remain pending. |
