...
| Worker benchmark runs real monitor/media probes from a hashed exported-state scenario, reports bounded per-protocol validation outcomes, and can fail closed on full coverage plus two starts per stream within bounded initial/repeat/trailing cycle and conservative wall-time gaps | P1 | implemented harness/live cadence and protocol-calibration runs; independent-source capacity run missing | `python3 -m unittest tests.test_worker_benchmark`; `python3 -m videosim worker-benchmark --scenario <state.json> --require-full-validation-coverage --max-validation-gap-cycles <cycles> --max-validation-gap-seconds <seconds> --json` |
| Fixture fleet serves healthy/slow/dead/malformed DASH/SRT URLs, validates optional per-behavior endpoint counts, and writes deterministic benchmark states | P1 | 500+500 live smoke passed; 660+660 saturation input ran but is shared-source and not admitted | `python3 -m unittest tests.test_fixture_fleet`; `python3 -m videosim fixture-fleet --manifest scale/fixtures/<protocol>-endpoints-660.json --state-path <state.json>` |
| Fixture scenario composes exact seeded protocol/behavior mixes, consumes multiple endpoints per behavior when supplied, and derives whether logical streams share protocol URLs | P1 | 1,000 and 1,320 distinct-URL inputs composed; ten-worker 1,320 run failed freshness, and independent generation/admission remain | `python3 -m unittest tests.test_fixture_scenario`; `python3 -m videosim fixture-scenario --manifest scale/fixtures/mixed-1320.json --srt-state <state.json> --dash-state <state.json> --state-path <state.json>` |
| F5 capacity verifier rejects incomplete policy criteria, weak workload/headroom/failure-domain declarations, dirty/failed/skipped runs, path escapes, missing artifacts, and SHA-256 drift | P0 | verifier and exact three-domain candidate contract implemented; approved run/evidence missing | `python3 -m unittest tests.test_scale_evidence`; `python3 -m videosim capacity-check --report <evidence.json> --policy scale/policies/f5-1000.json` |
| Durable scheduler enforces total plus SRT/DASH admission tokens, evenly fills bounded workers, and limits the checked-in 1,320-stream candidate's 11-worker domain loss to the failed 440 assignments | P0 | implemented model and local 33-container/PostgreSQL smoke; multi-host/media/24-hour evidence pending | `python3 -m unittest tests.test_gui tests.test_scale_evidence tests.test_worker tests.test_cli_video_feed`; see `docs/f5-local-domain-loss-smoke.md` |
| Compose exposes a sample worker node service | P1 | implemented | `docker compose config --quiet` |
| Candidate worker-domain Compose renders 11 unique mTLS worker IDs with exact 60/30/30 admission and 12/2 probe limits, bounded encrypted spools, and hardened containers | P0 | implemented and three local domains boot/failure-smoked; independent-host evidence pending | `python3 -m unittest tests.test_worker_domain_compose tests.test_cert_script`; `docker compose --env-file .env.worker-domain.example -f docker-compose.worker-domain.yml config --quiet` |
...
| Independent heartbeats publish schema-bounded cycle/deferral/spool pressure and durable operator state exposes the latest worker snapshot | P0 | implemented unit/integration | `python3 -m unittest tests.test_worker tests.test_gui`; `VIDEOSIM_TEST_POSTGRES_URL=... python3 -m unittest tests.test_postgres_store` |
...
| Durable assignment excludes spool-blocked workers, preserves fresh owners up to balanced targets, limits modeled 1,300-item worker loss to 130 required moves, rebalances on join, and recovers after DB-time worker/lease expiry with a higher epoch and stale-report rejection | P0 | implemented unit/integration plus 64.709-second local domain-loss smoke with zero healthy-owner moves; production partition/media evidence pending | `python3 -m unittest tests.test_gui`; `VIDEOSIM_TEST_POSTGRES_URL=... python3 -m unittest tests.test_postgres_store tests.test_worker_v2`; see `docs/f5-local-domain-loss-smoke.md` |
| Worker offer/renew and acknowledgement arrays use one ordered atomic PostgreSQL transaction and at most two SQL statements per request (worker fence plus set-based lease write); a late invalid tuple rolls back earlier lease changes | P0 | implemented integration with 1,000-row batch coverage; report-ingestion saturation remains open | `VIDEOSIM_TEST_POSTGRES_URL=... python3 -m unittest tests.test_postgres_store tests.test_worker_v2` |
| Durable assignment reads and lease writes share one tenant-scoped PostgreSQL advisory-lock transaction; backend loss rolls back before a second store takes leadership | P0 | implemented integration; replicated deployment and failover SLO pending | `VIDEOSIM_TEST_POSTGRES_URL=... python3 -m unittest tests.test_postgres_store.PostgresControlPlaneIntegrationTest.test_scheduler_transaction_rolls_back_and_releases_after_connection_loss tests.test_worker_v2` |
