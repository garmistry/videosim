...
| Fixture fleet serves healthy/slow/dead/malformed DASH/SRT URLs, validates optional per-behavior endpoint counts, and writes deterministic benchmark states | P1 | 500+500 live smoke passed; exact 220+220 per-domain shape implemented; 660+660 single-host saturation ran but is shared-source and not admitted | `python3 -m unittest tests.test_fixture_fleet`; `python3 -m videosim fixture-fleet --manifest scale/fixtures/<protocol>-endpoints-220-domain.json --state-path <state.json>` |
...
| Candidate fixture-domain Compose boots SRT/DASH load services on Linux host networking and startup validation checks the DASH API, real SRT/DASH media, process state, and Docker logs | P1 | render plus lightweight marked startup passed; three hosts and live 220+220 load per host pending | `python3 -m unittest tests.test_fixture_domain_compose`; `VIDEOSIM_FIXTURE_STARTUP_INTEGRATION=1 python3 -m unittest tests.test_fixture_domain_compose` |
...
| Durable control-plane load harness requires an empty disposable PostgreSQL database, creates the exact workload-driven worker/feed/lease shape, sustains heartbeats and profile-cadenced fenced reports, executes one declared worker-domain loss through real freshness expiry and production scheduling, fences a stale failed-owner report, records latency/resource/database metrics, and retains accepted-result outbox evidence | P0 | exact one-tick and compressed loss coverage implemented; the configured 30-second candidate default recovered all 440 affected streams onto 22 survivors at 29.888/29.889-second p95/p99 with zero healthy-owner churn and stale-report rejection; endpoint faults/media and the 24-hour production-like run remain pending | `VIDEOSIM_TEST_POSTGRES_URL=... python3 -m unittest tests.test_control_plane_load tests.test_cli_control_plane_load tests.test_worker_v2`; `python3 -m videosim control-plane-load --database-url <url> --workload scale/workloads/f5-1000-candidate.json --duration 24h --worker-freshness-seconds <deployed-value> --output <report.json>` |
...
| Candidate worker-domain Compose renders 11 unique mTLS worker IDs with exact 60/30/30 admission and 12/2 probe limits, bounded encrypted spools, and hardened containers; startup validates candidate identities/certificates, immutable image resolution, host plus worker-path health APIs, stable processes, and Docker logs | P0 | render and host-local durable API startup passed; immutable-image HTTPS/mTLS deployment on three independent hosts remains pending | `python3 -m unittest tests.test_worker_domain_compose tests.test_worker_domain_startup tests.test_cert_script`; `VIDEOSIM_WORKER_DOMAIN_STARTUP_INTEGRATION=1 python3 -m unittest tests.test_worker_domain_startup.WorkerDomainStartupIntegrationTest` |
