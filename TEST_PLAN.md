...
| Durable fixture startup gate boots fixture services, fresh PostgreSQL, migrated/imported catalog, API, and real workers; requires exact lease balance, latest per-behavior media outcomes, alarm shape, paginated API parity, stable containers, resources, and clean Docker logs | P0 | fast contract and marked eight-path/two-worker Docker workflow pass; an exploratory same-host 440-path run retained 28 inconclusive healthy-SRT timeouts, so full-shape local admission did not pass | `python3 -m unittest tests.test_durable_fixture_startup`; `VIDEOSIM_DURABLE_FIXTURE_STARTUP_INTEGRATION=1 VIDEOSIM_DURABLE_FIXTURE_IMAGE=<image> python3 -m unittest tests.test_durable_fixture_startup` |
