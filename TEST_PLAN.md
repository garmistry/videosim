...
| Fixture scenario composes exact seeded protocol/behavior mixes into 1,000 unique logical stream IDs while declaring shared physical endpoints and no capacity claim | P1 | implemented bounded-worker input; independent load pending | `python3 -m unittest tests.test_fixture_scenario`; `python3 -m videosim fixture-scenario --manifest scale/fixtures/mixed-1000.json --srt-state <state.json> --dash-state <state.json> --state-path <state.json>` |
