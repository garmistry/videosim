...
| F5 cross-domain startup preflight requires three clean digest-pinned fixture result/state triplets and three clean worker results; verifies exact 1,320 protocol/behavior paths, unique advertised hosts/endpoints, exact 33 worker IDs/zones, startup checks, timestamps, hashes, and one image digest without claiming physical independence or capacity | P0 | implemented with passing CLI coverage plus reused-host, state-tamper, image-drift, and non-F5-workload rejection; real three-host artifact set pending | `python3 -m unittest tests.test_f5_domain_preflight`; `python3 scripts/f5-domain-preflight.py --help` |
