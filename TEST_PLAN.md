...
| Worker benchmark runs real monitor/media probes from a hashed exported-state scenario and can fail closed on full validation coverage plus two starts per stream within bounded initial/repeat/trailing cycle and conservative wall-time gaps | P1 | implemented harness/live 1,000-URL cadence run; independent-source capacity run missing | `python3 -m unittest tests.test_worker_benchmark`; `python3 -m videosim worker-benchmark --scenario <state.json> --require-full-validation-coverage --max-validation-gap-cycles <cycles> --max-validation-gap-seconds <seconds> --json` |
