...
| Durable assignment excludes spool-blocked workers, preserves fresh owners up to balanced targets, limits modeled 1,300-item worker loss to 130 required moves, rebalances on join, and recovers after DB-time worker/lease expiry with a higher epoch and stale-report rejection | P0 | implemented unit/one-second PostgreSQL TTL path; production partition/timing and media evidence pending | `python3 -m unittest tests.test_gui`; `VIDEOSIM_TEST_POSTGRES_URL=... python3 -m unittest tests.test_postgres_store tests.test_worker_v2` |
| Worker offer/renew and acknowledgement arrays use one ordered atomic PostgreSQL transaction per request; a late invalid tuple rolls back earlier lease changes | P0 | implemented integration; production DB saturation pending | `VIDEOSIM_TEST_POSTGRES_URL=... python3 -m unittest tests.test_postgres_store tests.test_worker_v2` |
