...
| Durable assignment excludes spool-blocked workers, reports all-blocked shortfall, preserves fresh owners up to balanced targets, limits modeled 1,300-item worker loss to 130 required moves, and still rebalances on join | P0 | implemented unit/PostgreSQL path; partition/TTL and media evidence pending | `python3 -m unittest tests.test_gui`; `VIDEOSIM_TEST_POSTGRES_URL=... python3 -m unittest tests.test_postgres_store tests.test_worker_v2` |
