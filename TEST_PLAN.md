...
| Durable configuration forms require a positive client config version; any replica may update/alert/delete an external feed, stale update/delete returns HTTP 409 without mutation, and non-owning replicas reject generated-feed changes | P0 | implemented integration plus local two-API/1,320-feed mutation smoke; generated runtime ownership and idempotent create remain open | `VIDEOSIM_TEST_POSTGRES_URL=... python3 -m unittest tests.test_worker_v2.DurableWorkerV2ApiIntegrationTest.test_external_mutations_route_across_replicas_and_fence_stale_forms`; see `docs/f5-local-domain-loss-smoke.md` |
