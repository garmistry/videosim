...
| Fixture fleet serves healthy/slow/dead/malformed DASH/SRT URLs, validates optional per-behavior endpoint counts, and writes deterministic benchmark states | P1 | four-endpoint live smoke plus 500-URL manifests implemented; full 1,000 endpoint run/soak pending | `python3 -m unittest tests.test_fixture_fleet`; `python3 -m videosim fixture-fleet --manifest scale/fixtures/<protocol>-endpoints-500.json --state-path <state.json>` |
