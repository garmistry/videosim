...
| Fixture fleet serves a real healthy DASH feed plus deterministic slow, dead, and malformed endpoints and writes a four-stream worker-benchmark state | P1 | implemented DASH slice; SRT pending | `python3 -m unittest tests.test_fixture_fleet`; `python3 -m videosim fixture-fleet --manifest scale/fixtures/dash-matrix.json --state-path <state.json>` |
