...
| Durable control-plane load harness requires an empty disposable PostgreSQL database, creates the exact workload-driven worker/feed/lease shape, sustains heartbeats and profile-cadenced fenced reports, executes one declared worker-domain loss through real freshness expiry and production scheduling, fences a stale failed-owner report, records latency/resource/database metrics, and retains accepted-result outbox evidence | P0 | exact one-tick 1,320-stream/33-worker coverage plus compressed 11-worker loss/440-move/22-survivor recovery coverage implemented; zero healthy-owner churn and stale-report rejection proven; current 60-second control-plane default misses the 45-second p95 gate, endpoint faults/media and the 24-hour production-like run remain pending | `VIDEOSIM_TEST_POSTGRES_URL=... python3 -m unittest tests.test_control_plane_load tests.test_cli_control_plane_load`; `python3 -m videosim control-plane-load --database-url <url> --workload scale/workloads/f5-1000-candidate.json --duration 24h --worker-freshness-seconds <deployed-value> --output <report.json>` |
