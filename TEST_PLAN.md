...
| Worker benchmark runs real monitor/media probes from a hashed exported-state scenario and reports cycle/CPU percentiles, outcomes, peak RSS, and descriptors without claiming capacity | P1 | implemented harness; representative run missing | `python3 -m unittest tests.test_worker_benchmark`; `python3 -m videosim worker-benchmark --scenario <state.json> --json` |
