...
| Fixture fleet serves healthy/slow/dead/malformed DASH/SRT URLs, validates optional per-behavior endpoint counts, and writes deterministic benchmark states | P1 | 500 SRT plus 500 DASH URLs live-smoke-tested together; shared sources and soak remain | `python3 -m unittest tests.test_fixture_fleet`; `python3 -m videosim fixture-fleet --manifest scale/fixtures/<protocol>-endpoints-500.json --state-path <state.json>` |
| Fixture scenario composes exact seeded protocol/behavior mixes, consumes multiple endpoints per behavior when supplied, and derives whether logical streams share protocol URLs | P1 | 1,000 distinct live URLs composed; independent generation/load admission pending | `python3 -m unittest tests.test_fixture_scenario`; `python3 -m videosim fixture-scenario --manifest scale/fixtures/mixed-1000.json --srt-state <state.json> --dash-state <state.json> --state-path <state.json>` |
