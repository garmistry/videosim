...
| Durable control-plane load harness requires an empty disposable PostgreSQL database, creates the exact workload-driven worker/feed/lease shape, sustains heartbeats and profile-cadenced fenced reports, records latency/resource/database metrics, and retains accepted-result outbox evidence | P0 | implemented with exact one-tick 1,320-stream/33-worker/2,640-result/33-outbox PostgreSQL coverage; 24-hour production-like run pending | `VIDEOSIM_TEST_POSTGRES_URL=... python3 -m unittest tests.test_control_plane_load tests.test_cli_control_plane_load`; `python3 -m videosim control-plane-load --database-url <url> --workload scale/workloads/f5-1000-candidate.json --duration 24h --output <report.json>` |
