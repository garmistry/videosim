...
| F5 capacity verifier rejects incomplete policy criteria, weak workload/headroom/failure-domain declarations, dirty/failed/skipped runs, path escapes, missing artifacts, SHA-256 drift, and a missing or mismatched passed F5 domain-preflight report | P0 | verifier binds the exact three-domain preflight report to the evidence workload and container image; approved run/evidence missing | `python3 -m unittest tests.test_scale_evidence`; `python3 -m videosim capacity-check --report <evidence.json> --policy scale/policies/f5-1000.json` |
