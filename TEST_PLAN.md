...
| Worker benchmark runs real monitor/media probes from a hashed exported-state scenario and can fail closed on full validation coverage plus two starts per stream within a bounded initial/repeat/trailing cycle gap | P1 | implemented harness/shared-endpoint run; independent representative run missing | `python3 -m unittest tests.test_worker_benchmark`; `python3 -m videosim worker-benchmark --scenario <state.json> --require-full-validation-coverage --max-validation-gap-cycles <cycles> --json` |
| Fixture fleet serves real healthy DASH/SRT feeds plus deterministic slow, dead, and malformed endpoints and writes four-stream worker-benchmark states | P1 | implemented one-cycle fixtures; scale mix/soak pending | `python3 -m unittest tests.test_fixture_fleet`; `python3 -m videosim fixture-fleet --manifest scale/fixtures/<protocol>-matrix.json --state-path <state.json>` |
| Fixture scenario composes exact seeded protocol/behavior mixes, consumes multiple endpoints per behavior when supplied, and derives whether logical streams share protocol URLs | P1 | implemented composer; checked-in input still shares eight endpoints and independent load remains pending | `python3 -m unittest tests.test_fixture_scenario`; `python3 -m videosim fixture-scenario --manifest scale/fixtures/mixed-1000.json --srt-state <state.json> --dash-state <state.json> --state-path <state.json>` |
