...
| F3 — scalable workers | Independent heartbeat; bounded protocol-aware concurrency; deadlines/cancellation; capacity tokens; backpressure; encrypted spool; drain | Worker death/partition/ABA, slow-stream storm, spool recovery, fairness, and freshness evidence | Partial: durable `capacity.maxStreams` admission, bounded stream-level concurrency, and budget-based lower-priority deferral are implemented; hard cancellation, fairness, backpressure, spool, and drain controls remain open |
...
| Slow-stream containment | `--stream-budget-seconds` reports exhausted streams as timeout and defers remaining checks without clearing prior alarms | Implemented and unit-tested; in-flight cancellation and slow-stream-storm evidence remain open |
