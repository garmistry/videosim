...
| DASH/SRT endpoint fixture matrices | Existing generators plus HTTP/GStreamer fault services provide healthy, 30-second slow, dead, and malformed endpoints with bounded per-behavior URL counts; scaled SRT listeners share one encoded multicast source | Simultaneous 500-SRT/500-DASH Linux smoke implemented with 400/400 healthy SRT media passes and 400/400 DASH MPDs; shared-source, reconnect-storm, cross-product, and long-run evidence missing |
| Mixed fixture scenario | Seeded exact allocation composes protocol/behavior mixes from hashed fixture states, rotates across multiple URLs per behavior when supplied, and reports actual endpoint sharing | Live 500+500 states composed into 1,000 unique URLs with exact protocol/behavior counts; this is not independent generation or capacity evidence |
