...
| Operator feed read model | Viewer-authorized versioned API reads the durable feed catalog with a bounded keyset cursor instead of a process startup cache | Implemented with 100-row default/200-row maximum, fail-closed row validation, SQLite fallback, and a local alternating two-API 1,320-feed scan; GUI adoption, snapshot cursors, mutation routing, runtime ownership, and HA remain open |
