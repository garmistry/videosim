...
|---|---|---|---|
| F0 — contract and measurement foundation | Versioned worker contract; assignment generation/token; strict report ownership; worker-state pruning; independent heartbeat; probe instrumentation; deterministic control-plane benchmark | Rebalance, empty-assignment, stale-instance/generation/token, malformed/out-of-scope report, heartbeat-over-TTL, persistence failure, and concurrency tests; benchmark invariant report | Partial: local contracts/benchmark, batch CPU/peak-RSS/FD instrumentation, and scale-evidence verifier implemented; representative workload/resource report missing |
