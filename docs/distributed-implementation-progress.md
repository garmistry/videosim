...
| F4 — HA and data-plane split | Replicated stateless APIs; leader-elected scheduler; HA DB/broker; generated-feed service; dedicated DASH origin; SRT allocation; observability | API/scheduler/storage/zone failover, rolling upgrade/rollback, game-day, and operator runbook evidence | Partial: tenant-scoped PostgreSQL transaction leadership and connection-loss rollback are tested; two local API processes read one current durable external-feed assignment catalog, serialize scheduler bursts, serve bounded React reads, restart without catalog preload, allocate collision-resistant durable create IDs, and accept version-fenced external update/alert/delete on either replica; three local 11-worker domains passed hard-loss plus whole-domain network-partition control-plane smokes. Generated runtime ownership, persistent leadership, HA control/storage services, data-plane split, and production failover evidence remain open |
| F5 — 1,000-stream admission | Production-like workload manifest and immutable evidence bundle | 24-hour run, 30% headroom after one failure-domain loss, all audit Section 13.3 criteria | Policy/verifier, exact 33-worker/three-domain candidate, scheduler-loss model, and per-domain Compose config implemented; two API processes started after 1,320 external rows already existed with zero cached feeds, then returned all rows in 14 bounded pages and committed two unique concurrent creates; local repeated hard loss recovered in 65.466/66.018 seconds and a local same-process partition recovered in 61.194 seconds with only 440 required moves and zero healthy authority changes. Calibrated media freshness failed and the candidate remains unapproved/unrun on three hosts |
...
| Operator feed read model | Viewer-authorized versioned API reads the durable feed catalog with a bounded keyset cursor instead of a process startup cache | Implemented with 100-row React/root bootstrap, 200-row API maximum, lightweight overview, one-feed scoped detail, fail-closed row validation, SQLite fallback, and a local alternating two-API 1,320-feed scan; snapshot cursors, runtime ownership, and HA remain open |
| Durable API restart/create | PostgreSQL API process state remains empty across restart and create IDs do not depend on replica-local catalog state | Implemented with no durable preload, UUID-backed IDs, expected-version collision rejection, 40-create integration coverage, and a two-replica post-seed 1,320-feed smoke; idempotent create/retry and runtime ownership remain open |
| Replica-safe external mutations | Durable forms require a positive client config version and PostgreSQL performs atomic compare-and-swap plus success audit from any API replica | Implemented for external update/alert/delete with HTTP 400/409 contracts, generated-owner rejection, and a zero-cache two-API/1,320-feed smoke; generated runtime ownership and idempotent create remain open |
