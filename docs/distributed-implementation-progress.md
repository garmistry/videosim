...
| Pressure-aware placement | Durable scheduler gives spool-blocked workers no assignments and reports unmet demand; 1,300-item modeled headroom redistributes after one of ten workers is blocked | Implemented and unit-tested; no hysteresis, durable queue, or media/headroom certification |
...
| Validation rotation coverage | Opt-in benchmark gate requires every running assignment to start validation during measured cycles without exporting ID sets in worker reports | Implemented and 1,000-stream/125-cycle Linux-tested; shared endpoints and pressure budgets, not freshness/capacity evidence |
...
| Event transport | Hash-immutable transactional outbox, concurrent `SKIP LOCKED`, retry/dead state, exact JetStream policy, at-least-once IDs, atomic consumer inbox/ledger | Producer and dedicated replay consumer implemented and local integration-tested; replay remains an additive ledger, not event-driven read authority |
...
| Deployment | Production Compose role-init → migrate → role-grants → app/publisher/replay-consumer/pruner dependencies and persistent volumes | Implemented/config validated; PostgreSQL/NATS and the replay consumer remain single-instance |
