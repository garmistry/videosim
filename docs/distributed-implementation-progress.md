...
| Pressure-aware stable placement | Durable scheduler gives spool-blocked workers no assignments, retains fresh owners up to balanced total/protocol targets, reports unmet demand, and limits modeled 1,300-item worker loss to the failed worker's 130 assignments | Implemented and one-second PostgreSQL expiry-path tested; no hysteresis, durable queue, production partition/recovery timing, or media/headroom certification |
| Process-local failure benchmark | Removes configured workers after warmup, requires complete unique survivor ownership, rejects stale failed-worker reports, and reports minimum/excess assignment moves | Implemented and 1,300-stream-tested; current local round robin produced 1,044 excess moves, and durable TTL/partition/failure-domain evidence is missing |
| Scale evidence integrity | F5 policy/verifier requires all ten admission criteria, workload duration/headroom/failure-domain survivor tokens, clean successful runs, relative hashed artifacts, and no skipped checks | Implemented and fail-closed tested; no production-like evidence bundle exists |
...
| Worker resource benchmark | Hashed exported-state scenario drives real monitor probes and reports cycle/CPU percentiles, outcomes, unique validation-start coverage, peak RSS, and descriptors | Implemented and unit-tested; independent scenario, saturation curve, and fleet evidence missing |
| DASH/SRT endpoint fixture matrices | Existing generators plus HTTP/GStreamer fault services provide healthy, 30-second slow, dead, and malformed endpoints with exported benchmark states | Implemented and Linux-smoke-tested; source cross-product, storms, and long-run evidence missing |
| Mixed fixture scenario | Seeded exact allocation composes protocol/behavior mixes from hashed fixture states, rotates across multiple URLs per behavior when supplied, and reports actual endpoint sharing | Implemented and unique-URL unit-tested; checked-in 500/500, 800/100/50/50 input still has eight shared endpoints and is not capacity evidence |
| Validation rotation cadence | Opt-in benchmark gates require every running assignment to start validation, start at least twice, and remain within a bounded initial/repeat/trailing cycle gap without exporting ID sets in worker reports | Implemented and 1,000-stream/250-cycle Linux-tested with a 125-cycle maximum gap; shared endpoints/cycle cadence are not wall-time freshness or capacity evidence |
...
| Batched lease lifecycle | Worker assignment offer/renew and acknowledgement arrays each use one ordered atomic PostgreSQL transaction while retaining single-lease wrappers | Implemented and integration-tested; per-stream SQL, multi-controller leadership, and production DB saturation remain open |
| Scheduler transaction leadership | Durable assignment membership/owner reads, offer/renew, and revocation share one tenant-scoped PostgreSQL advisory-lock transaction | Implemented and two-store connection-loss tested; persistent leader, replicated deployment, lock-wait telemetry, and failover SLO remain open |
