...
| F5 — 1,000-stream admission | Production-like workload manifest and immutable evidence bundle | 24-hour run, 30% headroom after one failure-domain loss, all audit Section 13.3 criteria | Policy/capacity verifier, exact 33-worker/three-domain candidate, durable PostgreSQL load harness, read-only assignment/alarm evidence verifiers, scheduler-loss model, worker-domain Compose, and three-shard fixture-domain Compose/startup contracts implemented. The configured 30-second freshness/five-second heartbeat defaults recovered all 440 affected streams onto 22 survivors at 29.888/29.889-second p95/p99 in exact synthetic PostgreSQL evidence with zero healthy-owner churn and stale-report fencing. Calibrated media freshness still failed, and the candidate remains unapproved/unrun for 24 hours on three hosts |
