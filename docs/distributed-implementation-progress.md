...
| Pressure-aware stable placement | Durable scheduler gives spool-blocked workers no assignments, retains schedulable fresh owners within capacity during incomplete recovery, reports unmet demand, and limits modeled loss to unavailable ownership | Implemented with modeled and local 33-container/PostgreSQL loss/rejoin/second-loss coverage: each failed domain's 440 owners moved, all 880 healthy authority tuples stayed, and recovery completed in 65.466/66.018 seconds; live partition, separate-host timing, and media/headroom certification remain open |
