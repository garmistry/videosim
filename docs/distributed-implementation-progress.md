...
| F3 — scalable workers | Independent heartbeat; bounded protocol-aware concurrency; deadlines/cancellation; capacity tokens; backpressure; encrypted spool; drain | Worker death/partition/ABA, slow-stream storm, spool recovery, fairness, and freshness evidence | Partial: durable total/SRT/DASH admission, concurrency-window probe submission, rotating aggregate-budget validation deferral, validation-first phase ordering, staggered deep-check cadence, per-stream deadlines, bounded encrypted write-ahead report spooling, hard bounds for built-in media subprocess/poll waits, and final-report-ordered fenced drain are implemented; weighted check-cost/tenant fairness, durable fleet-queue backpressure, spool chaos/key rotation, and hard-kill evidence remain open |
...
| Bounded execution | `--max-concurrent-checks` limits simultaneous stream checks and executor submissions to one concurrency-sized window while preserving per-stream check order | Implemented and barrier-tested; no media-capacity certification |
