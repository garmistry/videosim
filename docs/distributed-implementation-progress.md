...
| F4 — HA and data-plane split | Replicated stateless APIs; leader-elected scheduler; HA DB/broker; generated-feed service; dedicated DASH origin; SRT allocation; observability | API/scheduler/storage/zone failover, rolling upgrade/rollback, game-day, and operator runbook evidence | Partial: tenant-scoped PostgreSQL transaction leadership and connection-loss rollback tested locally, and a reusable hardened 11-worker domain Compose file renders; replicated control/storage services, persistent leadership, data-plane split, and production failover evidence remain open |
| F5 — 1,000-stream admission | Production-like workload manifest and immutable evidence bundle | 24-hour run, 30% headroom after one failure-domain loss, all audit Section 13.3 criteria | Policy/verifier, exact 33-worker/three-domain candidate, scheduler-loss model, and per-domain Compose config implemented; calibrated 22-survivor single-host run failed freshness, and the candidate remains unapproved and unrun on three hosts |
...
| Pressure-aware stable placement | Durable scheduler gives spool-blocked workers no assignments, retains fresh owners up to balanced total/protocol targets, reports unmet demand, and limits modeled loss to unavailable ownership | Implemented with the checked-in candidate's modeled 1,320-stream/11-worker domain loss limited to 440 required moves and a separate one-second PostgreSQL expiry path; no live domain partition, durable queue, production timing, or media/headroom certification |
...
| Startup evidence | Isolated Compose app/worker boot followed by API create/start/normal validation/stop, with process state and Docker logs retained | Local workflow implemented and repeatedly run; three-host worker-domain boot/API/Chrome/log workflow documented and config-tested but not executed |
