...
| Startup evidence | Isolated Compose app/worker boot followed by API create/start/normal validation/stop, with process state and Docker logs retained; fixture and 11-worker domain workflows add media or identity/image/process checks | Local workflows implemented and repeatedly run. Worker-domain startup now emits uniform certificate, config, image, API, process/restart, and log evidence and passed against one durable local domain; immutable-image HTTPS/mTLS independent-host and authenticated production Chrome/OIDC evidence remains open |
