...
| F4 — HA and data-plane split | Replicated stateless APIs; leader-elected scheduler; HA DB/broker; generated-feed service; dedicated DASH origin; SRT allocation; observability | API/scheduler/storage/zone failover, rolling upgrade/rollback, game-day, and operator runbook evidence | Partial: tenant-scoped PostgreSQL transaction leadership and connection-loss rollback are tested, and three local 11-worker domains passed a hard-loss control-plane smoke; replicated control/storage services, persistent leadership, data-plane split, and production failover evidence remain open |
| F5 — 1,000-stream admission | Production-like workload manifest and immutable evidence bundle | 24-hour run, 30% headroom after one failure-domain loss, all audit Section 13.3 criteria | Policy/verifier, exact 33-worker/three-domain candidate, scheduler-loss model, and per-domain Compose config implemented; the first local PostgreSQL loss recovered 1,320 leases in 64.709 seconds with only 440 required moves, while a report-ingestion follow-up recovered in 66.396 seconds with one healthy move and no HTTP 499/5xx; calibrated media freshness failed and the candidate remains unapproved/unrun on three hosts |
...
| Pressure-aware stable placement | Durable scheduler gives spool-blocked workers no assignments, retains fresh owners up to balanced total/protocol targets, reports unmet demand, and limits modeled loss to unavailable ownership | Implemented with modeled coverage plus a local 33-container/PostgreSQL hard-loss smoke: 440 failed owners moved, 880 survivor owners stayed, and recovery completed in 64.709 seconds; no live partition, separate-host timing, or media/headroom certification |
