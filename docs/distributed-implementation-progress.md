...
| F5 — 1,000-stream admission | Production-like workload manifest and immutable evidence bundle | 24-hour run, 30% headroom after one failure-domain loss, all audit Section 13.3 criteria | Policy/capacity verifier, exact 33-worker/three-domain candidate, read-only assignment evidence verifier, scheduler-loss model, worker-domain Compose, and three-shard fixture-domain Compose/startup contracts implemented; PostgreSQL-backed assignment capture proves exact 1,320/33 baseline and 1,320/22 survivor authority with only 440 failed-domain moves; two API processes started after 1,320 external rows already existed with zero cached feeds, then returned all rows in 14 bounded pages and committed two unique concurrent creates; local repeated hard loss recovered in 65.466/66.018 seconds and a local same-process partition recovered in 61.194 seconds with only 440 required moves and zero healthy authority changes. Calibrated media freshness failed and the candidate remains unapproved/unrun on three hosts |
