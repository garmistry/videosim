...
| Worker resource benchmark | Hashed exported-state scenario drives real monitor probes and reports cycle/CPU percentiles, aggregate/per-protocol outcomes, unique validation-start coverage, conservative wall-time freshness bounds, peak RSS, and descriptors | Implemented and protocol-calibrated; one local 220-SRT run attempted every distinct path in 14 cycles and 155.947 seconds, while independent sources, a fleet saturation curve, and admission evidence remain missing |
| DASH/SRT endpoint fixture matrices | Existing generators plus HTTP/GStreamer fault services provide healthy, 30-second slow, dead, and malformed endpoints with bounded per-behavior URL counts; scaled SRT listeners share one encoded multicast source and batch up to 16 distinct listener sinks per relay pipeline | Simultaneous 500+500 smoke passed; one local exact 220+220 shard passed startup and physical two-container fault/recovery, and a later batched-listener run completed all 220 SRT path attempts with resource/process/log evidence. The prior 660+660 single-host saturation failed freshness; three-host all-path alarms/load, per-stream independence, reconnect storms, cross-product, and long-run evidence remain |
