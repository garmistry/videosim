...
| Durable control-plane load | Empty disposable PostgreSQL database receives workload-driven feeds/workers, exact balanced leases, independent heartbeats, profile-cadenced fenced synthetic results, current state, and immutable result-outbox evidence with latency/resource/database metrics | Implemented and one-tick exact 1,320-stream/33-worker PostgreSQL-tested; media probes, worker HTTP/mTLS, broker consumption, failure injection, saturation curve, and 24-hour evidence remain open |
| Assignment evidence integrity | One repeatable read-only PostgreSQL snapshot proves desired/protocol coverage, fresh worker-domain shape, exact advertised capacity, single current tenant-fenced authority, balanced placement, and baseline-bounded ownership changes | Implemented and exact 1,320-stream PostgreSQL-tested for 33-worker baseline plus 22-worker/440-move domain loss; independent-host captures and separate stale-report/failover/media evidence remain open |
