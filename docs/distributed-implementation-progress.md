...
| F5 — 1,000-stream admission | Production-like workload manifest and immutable evidence bundle | 24-hour run, 30% headroom after one failure-domain loss, all audit Section 13.3 criteria | Policy/capacity verifier, exact 33-worker/three-domain candidate, durable PostgreSQL load harness, read-only assignment/alarm evidence verifiers, scheduler-loss model, worker-domain Compose, and three-shard fixture-domain Compose/startup contracts implemented. The load harness proves an exact 1,320-stream/33-worker synthetic-probe tick and compressed real-expiry recovery of all 440 affected streams onto 22 survivors with zero healthy-owner churn and stale-report fencing. The current 60-second freshness default misses the 45-second p95 gate; calibrated media freshness also failed, and the candidate remains unapproved/unrun for 24 hours on three hosts |
