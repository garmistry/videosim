...
| F5 — 1,000-stream admission | Production-like workload manifest and immutable evidence bundle | 24-hour run, 30% headroom after one failure-domain loss, all audit Section 13.3 criteria | Policy/capacity verifier, exact 33-worker/three-domain candidate, durable PostgreSQL load harness, read-only assignment/alarm evidence verifiers, scheduler-loss model, worker/fixture-domain Compose, and host-local startup evidence workflows implemented. The configured 30-second freshness/five-second heartbeat defaults recovered all 440 affected streams onto 22 survivors at 29.888/29.889-second p95/p99 in exact synthetic PostgreSQL evidence with zero healthy-owner churn and stale-report fencing. One local worker-domain startup registered exact 11-worker capacity, but calibrated media freshness still failed and the candidate remains unapproved/unrun for 24 hours on three hosts |
...
| Durable control-plane load | Empty disposable PostgreSQL database receives workload-driven feeds/workers, exact balanced leases, independent heartbeats, profile-cadenced fenced synthetic results, current state, and immutable result-outbox evidence with latency/resource/database metrics; one declared worker-domain loss uses real freshness expiry and production scheduling before stale-report and survivor-report checks | Implemented for an exact 1,320-stream/33-worker tick plus 11-worker-loss/440-move/22-survivor recovery. The configured 30-second candidate default passed the 45/90-second control-plane gate at 29.888/29.889-second p95/p99; media probes, worker HTTP/mTLS domain loss, broker consumption, endpoint faults, saturation curve, independent hosts, and 24-hour evidence remain open |
