...
| DASH/SRT endpoint fixture matrices | Existing generators plus HTTP/GStreamer fault services provide healthy, 30-second slow, dead, and malformed endpoints with bounded per-behavior URL counts | Four-endpoint Linux smoke and exact 500-URL-per-protocol manifest tests implemented; full fleet, source cross-product, storms, and long-run evidence missing |
| Mixed fixture scenario | Seeded exact allocation composes protocol/behavior mixes from hashed fixture states, rotates across multiple URLs per behavior when supplied, and reports actual endpoint sharing | Implemented and unique-URL unit-tested; checked-in 500+500 URL manifests exist, but full live composition is not capacity evidence and DASH still shares one origin |
