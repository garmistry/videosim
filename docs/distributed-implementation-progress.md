...
| F5 — 1,000-stream admission | Production-like workload manifest and immutable evidence bundle | 24-hour run, 30% headroom after one failure-domain loss, all audit Section 13.3 criteria | Policy/capacity verifier, exact 33-worker/three-domain candidate, atomic fixture-scenario catalog import, durable PostgreSQL load harness, read-only assignment/alarm evidence verifiers, scheduler-loss model, worker/fixture-domain Compose, and host-local startup evidence workflows implemented. The configured 30-second freshness/five-second heartbeat defaults recovered all 440 affected streams onto 22 survivors at 29.888/29.889-second p95/p99 in exact synthetic PostgreSQL evidence with zero healthy-owner churn and stale-report fencing; the load harness also proves exact 1,320-stream synthetic alarm raise/recovery and outbox parity. A local import/API run proved exact 1,320 durable URL parity and one 220+220 source shard completed every SRT/DASH path attempt with behavior-level outcomes, but the prior 1,320 single-host media run failed freshness and the candidate remains unapproved/unrun for 24 hours on three hosts |
...
| DASH/SRT endpoint fixture matrices | Existing generators plus HTTP/GStreamer fault services provide healthy, 30-second slow, dead, and malformed endpoints with bounded per-behavior URL counts; scaled SRT listeners share one encoded multicast source and batch up to 16 distinct listener sinks per relay pipeline | Simultaneous 500+500 smoke passed; one local exact 220+220 shard passed startup and physical two-container fault/recovery, then sequential sweeps covered every SRT/DASH URL and separated healthy, slow, dead, and malformed outcomes. The prior 660+660 single-host saturation failed freshness; three-host durable alarms/load, per-stream independence, reconnect storms, cross-product, and long-run evidence remain |
...
| Startup evidence | Isolated Compose app/worker boot followed by API create/start/normal validation/stop, with process state and Docker logs retained; fixture and 11-worker domain workflows add media, inventory/resources, or identity/image/process checks; candidate catalog startup adds exact import and paginated Chrome/API parity | Local workflows implemented and repeatedly run. One exact 220+220 fixture shard with complete sequential SRT/DASH path sweeps, one exact 1,320 catalog/API startup, and one durable 11-worker domain passed locally; immutable production builds, HTTPS/mTLS independent hosts, full 1,320-path durable media/alarm load, and authenticated production Chrome/OIDC evidence remain open |
