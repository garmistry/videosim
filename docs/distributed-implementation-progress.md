...
| F5 — 1,000-stream admission | Production-like workload manifest and immutable evidence bundle | 24-hour run, 30% headroom after one failure-domain loss, all audit Section 13.3 criteria | Policy/capacity verifier, exact 33-worker/three-domain candidate, atomic fixture-scenario catalog import, durable PostgreSQL load harness, read-only assignment/alarm evidence verifiers, scheduler-loss model, worker/fixture-domain Compose, and fail-closed startup evidence workflows implemented. The configured 30-second freshness/five-second heartbeat defaults recovered all 440 affected streams onto 22 survivors at 29.888/29.889-second p95/p99 in exact synthetic PostgreSQL evidence with zero healthy-owner churn and stale-report fencing; the load harness also proves exact 1,320-stream synthetic alarm raise/recovery and outbox parity. A marked durable eight-path startup passes. The exploratory same-host exact-440 SRT count used a superseded detector that did not require caption bytes, so it is transport-only negative diagnosis; the candidate remains unapproved/unrun for 24 hours on three hosts |
...
| Durable control-plane load | Empty disposable PostgreSQL database receives workload-driven feeds/workers, exact balanced leases, independent heartbeats, profile-cadenced fenced synthetic results, current state, and immutable result/alarm-outbox evidence with latency/resource/database metrics; one declared worker-domain loss uses real freshness expiry and production scheduling before stale-report and survivor-report checks, and one declared endpoint storm commits separate all-stream fault/recovery windows | Implemented for an exact 1,320-stream/33-worker tick, 11-worker-loss/440-move/22-survivor recovery, and 1,320 raise plus 1,320 clear alarm burst with 2,640 matching outbox transitions. The configured 30-second candidate default passed the 45/90-second control-plane gate at 29.888/29.889-second p95/p99; media probes, worker HTTP/mTLS domain loss, broker consumption, real endpoint faults, saturation curve, independent hosts, and 24-hour evidence remain open |
| Assignment evidence integrity | One repeatable read-only PostgreSQL snapshot proves desired/protocol coverage, fresh worker-domain shape, exact advertised capacity, single current tenant-fenced authority, balanced placement, and baseline-bounded ownership changes | Implemented and exact 1,320-stream PostgreSQL-tested for 33-worker baseline plus 22-worker/440-move domain loss; control-plane load separately fences the stale failed-owner report, while independent-host physical timing and media evidence remain open |
...
| Worker resource benchmark | Hashed exported-state scenario drives real monitor probes and reports cycle/CPU percentiles, aggregate/per-protocol outcomes, bounded fixture-behavior coverage/outcomes, unique validation-start coverage, conservative wall-time freshness bounds, peak RSS, and descriptors | Implemented and protocol-calibrated; sequential local runs attempted all 220 SRT and all 220 DASH paths in 155.867 and 62.969 seconds with exact behavior coverage, while independent sources, a fleet saturation curve, and admission evidence remain missing |
| DASH/SRT endpoint fixture matrices | Existing generators plus HTTP/GStreamer fault services provide healthy, 30-second slow, dead, and malformed endpoints with bounded per-behavior URL counts; scaled SRT listeners share one encoded multicast source and use one downstream-leaky relay per live endpoint | Simultaneous 500+500 smoke passed; one local exact 220+220 shard passed startup and physical two-container fault/recovery, then sequential sweeps covered every SRT/DASH URL and separated healthy, slow, dead, and malformed outcomes. Concurrent sweeps rejected the prior multi-sink relay design; three-host durable alarms/load, source independence, saturation, cross-product, and long-run evidence remain |
...
| Fixture catalog import | Validate complete composed-state fields, require distinct URLs, atomically reconcile an exclusive PostgreSQL catalog through normal feed generations, verify persisted parity, and retain a non-capacity report | Implemented with rollback/idempotence and real scheduler-lease PostgreSQL tests. One local exact 1,320 import and seven-page API walk matched every source endpoint; independent-host worker media/alarm evidence remains open |
...
| Startup evidence | Isolated Compose app/worker boot followed by API create/start/normal validation/stop, with process state and Docker logs retained; fixture and 11-worker domain workflows add media, inventory/resources, or identity/image/process checks; the durable fixture workflow combines fresh database/import, real workers, all-path outcomes/alarms, paginated API, and Docker evidence | Local workflows implemented and repeatedly run. The corrected marked eight-path durable gate passes with byte-backed SRT caption evidence; the exact 440-path exploratory run used the superseded detector and is not essence evidence. Immutable production builds, HTTPS/mTLS independent hosts, full 1,320-path durable media/alarm load, and authenticated production Chrome/OIDC evidence remain open |
...
| Scale evidence | Workload-driven durable load report plus read-only assignment and alarm-consistency reports; production-like durability/load/failover data | Exact one-tick 1,320-stream PostgreSQL load/verifier coverage implemented; independent-host 24-hour evidence not started, so no scale claim |
