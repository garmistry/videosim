...
| F4 — HA and data-plane split | Replicated stateless APIs; leader-elected scheduler; HA DB/broker; generated-feed service; dedicated DASH origin; SRT allocation; observability | API/scheduler/storage/zone failover, rolling upgrade/rollback, game-day, and operator runbook evidence | Partial: tenant-scoped PostgreSQL transaction leadership and connection-loss rollback are tested; two local API processes read one current durable external-feed catalog and serialize scheduler bursts; three local 11-worker domains passed hard-loss plus whole-domain network-partition control-plane smokes. Replicated operator state, persistent leadership, HA control/storage services, data-plane split, and production failover evidence remain open |
| F5 — 1,000-stream admission | Production-like workload manifest and immutable evidence bundle | 24-hour run, 30% headroom after one failure-domain loss, all audit Section 13.3 criteria | Policy/verifier, exact 33-worker/three-domain candidate, scheduler-loss model, and per-domain Compose config implemented; two local API processes assigned all 1,320 external placeholders exactly once with zero scheduler retries or cross-replica ownership changes; local repeated hard loss recovered in 65.466/66.018 seconds and a local same-process partition recovered in 61.194 seconds with only 440 required moves and zero healthy authority changes. Calibrated media freshness failed and the candidate remains unapproved/unrun on three hosts |
